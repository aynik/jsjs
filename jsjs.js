...
(function(){
    ...
    // Save the previous value of jsjs
    var prevjsjs = root.jsjs;

    // Create the jsjs namespace
    var jsjs = {};

    // Define the main compiler
    jsjs.compile = compile;
    
    // Define the main reader
    jsjs.read = function(input, opts){
        opts = opts || {};
    ...
    function compile(flags, il, debug){
        flags = flags || { format: '  ' };
        il = il || 0; 
        debug = debug || function(type, node){
            console.log('-------------');
            console.log('debug ', type); 
            console.dir(node);
        };
        return function(node){
            var log = function(){ debug(node.type, node) };
            var rules = {  
                'Function': function(){
                ...
                'FunctionCallArguments': log,
                'PropertyAccessProperty': log,
